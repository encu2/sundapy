...
pub const Node = union(enum) {
    ...
    del_stmt: struct { target: *Node },
    nonlocal_decl: struct { name: []const u8 },
    ...
    tuple_assign: struct { targets: std.ArrayList(*Node), value: *Node },
    ...
    while_stmt: struct { condition: *Node, body: std.ArrayList(*Node), else_body: ?std.ArrayList(*Node) },
    with_stmt: struct { context_expr: *Node, as_name: ?[]const u8, body: std.ArrayList(*Node), is_async: bool },
    for_stmt: struct { is_async: bool, iterator: []const u8, iterable: *Node, body: std.ArrayList(*Node), else_body: ?std.ArrayList(*Node) },
    ...
    assert_stmt: struct { condition: *Node, message: ?*Node },
    yield_expr: struct { value: ?*Node },
    ...
    try_stmt: struct { body: std.ArrayList(*Node), except_branch: ?std.ArrayList(*Node), except_type: ?[]const u8, except_as: ?[]const u8, finally_branch: ?std.ArrayList(*Node), else_branch: ?std.ArrayList(*Node) },
    raise_stmt: struct { value: ?*Node, from_exc: ?*Node },
    ...
    none: void,
    ellipsis: void,
    dummy_expr: struct { desc: []const u8 },
};
